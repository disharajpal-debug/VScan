...
def setup_admin(email="admin@c4i4.org", password=None):
    ...
    if user:
        ...
        users_collection.update_one(
            {"email": email},
            {"$set": {"password": hashed, "role": "admin", "name": "Administrator"}},
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--email",
        default="admin@c4i4.org",
        help="Admin email (default: admin@c4i4.org)",
    )
    parser.add_argument(
        "--password", help="Admin password (auto-generated if not provided)"
    )
